...
def main_baseline():
    ...
    param = SamplingParams(temperature=1.0, top_p=1.0, max_generation_length=1024)
    param.stop = ["</answer>"]
...
def evaluate_gsm(
    ...
    test_data_path = os.path.join(os.path.dirname(__file__), "..", "data", "gsm8k", "train.jsonl")

    with open(test_data_path, "r") as f:
        test_data = [json.loads(line) for line in f]

    test_prompt_list = [prompt_template.format(**data) for data in test_data]
    ...
    output_path = os.path.join(os.path.dirname(__file__), "..", "exp_results", f"{exp_name}.jsonl")
